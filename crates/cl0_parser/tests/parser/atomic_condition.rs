use chumsky::Parser;
use cl0_parser::{ast::{AtomicCondition, PrimitiveCondition}, parser::atomic_condition_parser};
...
fn assert_parses_to(
    src: &str,
    want: AtomicCondition<'_>,
) {
    let tokens = lex_tokens(src);
    let parsed = atomic_condition_parser()
...
#[test]
fn atomic_var() {
    assert_parses_to("foo", AtomicCondition::Primitive(PrimitiveCondition::Var("foo")));
}
