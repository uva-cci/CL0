...
function getLiteralsByAngularAttribute(ctx, attribute, attributes) {
    const literals = attributes.reduce((literals, attributes) => {
        if ((0, readable_tailwind_utils_matchers_js_1.isAttributesName)(attributes)) {
            if (!(0, readable_tailwind_utils_utils_js_1.matchesName)(attributes.toLowerCase(), getAttributeName(attribute).toLowerCase())) {
                return literals;
            }
            ...
            if (!(0, readable_tailwind_utils_utils_js_1.matchesName)(attributes[0].toLowerCase(), getAttributeName(attribute).toLowerCase())) {
                return literals;
            }
...
function getLiteralsByAngularMatchers(ctx, ast, matchers) {
    ...
    const matchingAstNodes = (0, readable_tailwind_utils_matchers_js_1.getLiteralNodesByMatchers)(ctx, ast, matcherFunctions, value => isAST(value) && isCallExpression(value));
...
function createLiteralByLiteralMapKey(ctx, key) {
    ...
    const whitespaces = (0, readable_tailwind_utils_utils_js_1.getWhitespace)(keyContent);
...
function createLiteralByAngularTemplateLiteralElement(ctx, literal) {
    ...
    const indentation = (0, readable_tailwind_utils_utils_js_1.getIndentation)(parentLine);
    const supportsMultiline = getMultilineSupport(ctx);
    const multilineQuotes = supportsMultiline ? ["'", "\"", "`"] : ["'", "\""];
    return [{
            ...quotes,
